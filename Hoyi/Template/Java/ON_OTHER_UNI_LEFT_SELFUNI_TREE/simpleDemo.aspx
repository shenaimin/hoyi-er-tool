<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="lstrpg_<%%# e.ClassName #%%>.aspx.cs" Inherits="CTERP.page.mds.<%%# m.Caption #%%>.<%%# e.ClassName #%%>.lstrpg_<%%# e.ClassName #%%>" %>
							...
							<asp:Repeater runat="server" ID="rep1" OnItemCommand="rep1_ItemCommand">
								...
								<ItemTemplate>
									<tr>
										...
										<%%# [Fields[Equals(1)]={<td>
											<a target="_blank" href="lstrsw_<%#= e.ClassName #%>.aspx?op=show&paraid=<%# Eval(<%#= e.ClassName #%>Info.F_<%#= e.getfirfieldName() #%>)%>"><%# Eval(<%#= e.ClassName #%>Info.F_<%#= t.ColumnName #%>) %></a>
										</td>		}] #%%>
										<%%# [Fields[>2]={
										<%#= if(t.isunikey()){
