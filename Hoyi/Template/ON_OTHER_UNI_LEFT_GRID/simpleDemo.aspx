<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="leftpg_<%%# e.ClassName #%%>.aspx.cs" Inherits="CTERP.page.mds.<%%# m.Caption #%%>.<%%# e.ClassName #%%>.leftpg_<%%# e.ClassName #%%>" %>
					...
					<table id="mytable" style="float:left;font-size:12px;">
						...
						<tbody>
							<asp:Repeater runat="server" ID="rep1" OnItemCommand="rep1_ItemCommand">
								<HeaderTemplate>
									<tr>
										<%%# [Fields[Equals(0)]={<th> <asp:LinkButton runat="server" CommandArgument="<%# <%#= e.ClassName #%>Info.F_<%#= t.ColumnName #%> %>" OnClick="btnOrder_Click" ><%# OrderTag("<%#= t.Comment #%>", <%#= e.ClassName #%>Info.F_<%#= t.ColumnName #%>) %></asp:LinkButton></th> 		}] #%%>
